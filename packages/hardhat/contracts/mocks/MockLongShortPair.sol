...
import "@openzeppelin/contracts/token/ERC20/preset/ERC20PresetMinterPauser.sol";
import "../interfaces/ILongShortPair.sol";

contract MockLongShortPair is ILongShortPair {
    ...
    constructor(CreatorParams memory params) {
        collateral = params.collateral;
        longToken = new ERC20PresetMinterPauser(params.longSynthName, params.longSynthSymbol);
        shortToken = new ERC20PresetMinterPauser(params.shortSynthName, params.shortSynthSymbol);
    ...
    function create(uint256 tokensToCreate) external returns (uint256 collateralUsed) {
        collateral.transferFrom(msg.sender, address(this), tokensToCreate * collateralPerPair)
        longToken.mint(msg.sender, tokensToCreate);
        shortToken.mint(msg.sender, tokensToCreate);

        return tokensToCreate * collateralPerPair;
    }
